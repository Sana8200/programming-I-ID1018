	...
	public static void main (String[] args)
	{
		...
		for (int week = 1; week <= nofWeeks; week++) {
		    ...
		    for (int measurement = 2; measurement <= nofMeasurementsPerWeek; measurement++) {
		        double curTemperature = t[week][measurement];
		        
		        if (curTemperature < minT[week]) {
		            minT[week] = curTemperature;
		        }
		        if (curTemperature > maxT[week]) {
		            maxT[week] = curTemperature;
		        }
		        sumT[week] += curTemperature;
		    }
		...
		for (int week_in = 2; week_in <= nofWeeks; week_in++) {
		    if (minT[week_in] < minTemp) {
		        minTemp = minT[week_in];
		    }
		    if (maxT[week_in] > maxTemp) {
		        maxTemp = maxT[week_in];
		    }
		    sumTemp += sumT[week_in];
		}
